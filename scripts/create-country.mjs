...
async function writeContinuationBundle(outDir) {
  ...
  for (const source of ['docs/SOURCE_ADAPTER_GUIDE.md', 'docs/COMMON_DATA_AND_SOURCE_REGISTRY.md', 'docs/IMPLEMENTATION_CONTRACT.md', 'docs/PLANNING_DATA_CONTRACT.md', 'docs/ANALYSIS_DATA_CONTRACT.md', 'docs/CENSUS_SERIES_CONTRACT.md', 'docs/PLANNING_CENSUS_METHOD.md', 'docs/02_COMMON_SPEC.md', 'docs/03_COUNTRY_AND_DATA.md', 'templates/ACCEPTANCE.md', 'templates/COUNTRY_START.md', 'templates/TASK_AND_CHANGE.md', 'templates/PLANNING_CENSUS_AUDIT.md', 'templates/DELIVERY.json', 'templates/country-profile.json']) {
...
export async function createCountry({ country, out, collect = collectCountry, generate = generateSite }) {
  ...
  try {
    ...
    await writeFile(path.join(outDir, 'evidence', 'WORK_STATUS.json'), JSON.stringify({status:'research_required',country_id:dataset.country.id,prepared_at:new Date().toISOString(),message:'Internal workspace preparation completed. Do not show this site as the user delivery. Continue country research, law-aligned Word outline and source-link classification, integration, browser/output verification, then create DELIVERY.json and pass verify-delivery.mjs.'},null,2)+'\n');
    await writeFile(path.join(outDir, 'AGENTS.md'), `# Country project\n\nCountry: ${dataset.country.name} (${dataset.country.id}).\n\nThe user requested one finished country dashboard, not a staged handoff. Read COUNTRY_AGENT_WORKFLOW.md, evidence/SOURCE_PREFLIGHT.md, TEMPLATE_REFERENCE.json, the bundled reference/docs and reference/templates, and HANDOFF.md if present. The current generated files are an internal workspace only. Never present or open them as the final result while evidence/WORK_STATUS.json says research_required. Refresh country sources, inspect the official census catalogue and detailed tables, collect and integrate usable subnational statistics, reconcile codes and boundaries, research the planning system and actual materials, and verify the country plan index against law, regulations, official guidance and forms. Add planning.source_groups for law, census and international source links. Add a cited planning.document_template, adopt docx, generate a selected-area Word sample, render every page with the isolated renderer, and record the review. Adapt all three pages and verify browser behavior and downloads. Copy reference/templates/DELIVERY.json to evidence/DELIVERY.json only after the recorded checks are true, then run the template script verify-delivery.mjs. Do not return a completion message unless that command passes. SOURCE_PREFLIGHT is a discovery plan, not proof of acquisition or suitability. Never distribute national values across local territories. Keep census, sample survey, humanitarian observation and modeled grid values as different evidence types. Keep the DDPT page roles and synchronized selection. Reference: ${repositoryUrl}\n\nDo not overwrite the source template repository. Do not use host Microsoft Word COM. Publish only within the current user's requested destination and authorization.\n`);

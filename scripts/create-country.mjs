...
import { writeSourcePreflight, loadSourceCatalog, findCountrySourceRecord, findPriorityCountryRecord } from '../lib/source-catalog.mjs';
...
export async function createCountry({ country, out, collect = collectCountry, generate = generateSite }) {
  ...
  try {
    ...
    if (collect === collectCountry) {
      ...
      const resolved = countrySource || priority;
      if (resolved) {
        collectionCountry = resolved.iso3;
        identity = { iso3: resolved.iso3, iso2: priority?.iso2, name: countrySource?.names?.[0] || priority?.name_en || resolved.iso3 };
      }
    }

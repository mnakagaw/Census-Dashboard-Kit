...
export function diagnosticCsv(data,territoryId,period) {
  ...
  for(const indicator of data.indicators){
    const overall=overallEntry(data,parent,indicator,period);
    for(const [scope,entry] of [['overall',overall],...internalComparison(data,territoryId,indicator.id,period).rows.map(row=>['within_area',row])])rows.push([data.country.id,territoryId,parent.name,scope,entry.area.id,entry.area.name,entry.area.type,entry.area.code_system,entry.area.official_code,entry.area.parent_id,entry.area.boundary_version,indicator.id,indicator.name,entry.period || entry.observation?.period || period,entry.value,rowUnit(entry,indicator),entry.status,entry.provenance,entry.note,entry.components?.map(item=>item.territory_id).join('; '),entry.components?.map(item=>`${item.territory_id}@${item.period}`).join('; '),entry.missing_ids?.join('; '),entry.covered_value,entry.comparable,entry.reason,entry.definition_id,rowDefinition(entry,indicator),entry.population,entry.method,entry.meaning_comparable,observedBoundary(entry.observation),scope==='overall'?'Not applicable to an overall statistical record':entry.boundary_reason || 'Matched geometry',entry.source?.id,entry.source?.name,entry.source?.url,entry.source?.retrieved_at,data.generated_at]);
  }
  return makeCsv(rows);
}
...
export function diagnosticHtml(data,territoryId,period) {
  ...
  const sections=data.indicators.map(indicator=>{
    const result=overallEntry(data,area,indicator,period);
    return `<section><h2>${e(indicator.theme)} — ${e(indicator.name)}</h2><p><strong>Overall: ${e(valueText(data,result.value,indicator))} ${e(result.unit)}</strong> · ${e(statusLabel(result.status))} · ${e(period)}</p><p>${e(meaningText(result))}</p><p>Observation boundary edition: ${e(observedBoundary(result.observation))}.</p><p>Source: ${sourceLink(result.source)} · Retrieved ${e(result.source?.retrieved_at || 'not recorded')}.</p><h3>Acquired history</h3>${seriesReport(data,area,indicator)}${renderInternalComparison(data,territoryId,indicator.id,period,{interactive:false})}</section>`;
  }).join('');

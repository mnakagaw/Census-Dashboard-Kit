...
export function comparisonSummary(data,comparison) {
  const values=comparison.rows.filter(row=>row.comparable && finite(row.value)).map(row=>row.value);
  if(!values.length)return comparison.reason || 'No comparable lower-area observations are available for this indicator and period. The overall diagnosis remains available.';
...
export function renderInternalComparison(data,parentId,indicatorId,period,{interactive=true,language='en'}={}) {
  ...
  const parent=data.territories.find(row=>row.id===parentId),parentResult=parent?areaObservationState(data,parentId,indicatorId,period):null;
  const lowerAreaNotice=!finite(parentResult?.value)?`<p class="available-period-note"><strong>No value for the selected area as a whole.</strong> The map and table show ${comparableCount} of ${comparison.rows.length} lower areas with observations. They are not displayed as a value for ${e(parent?.name||parentId)}.</p>`:'';
  ...
  const map=geometry.paths.length?`<svg class="internal-map" viewBox="0 0 760 400" role="group" aria-label="${e(label)}. Inspect values without changing the analysis area."><title>${e(label)}</title><rect width="760" height="400" fill="#f4f8f7"/>${geometry.paths.map(path=>{
    const row=byId.get(path.id),text=`${row.area.name}: ${valueText(data,row.value,indicator)} ${rowUnit(row,indicator)} · ${evidenceStatus(indicator,row.observation,row.status)}${row.comparable?'':' · comparison not established'}`;
...
export function diagnosticMarkdown(data,territoryId,period) {
  ...
  for(const indicator of data.indicators) {
    ...
    if(!latestMode){
      lines.push('','### Acquired history','','| Period | Value | Unit | Status | Definition / population / method / comparison context | Observation boundary | Source / retrieved |','|---|---:|---|---|---|---|---|');

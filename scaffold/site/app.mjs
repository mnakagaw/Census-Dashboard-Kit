...
const comparisonLevelsFor = indicatorId => localLevels(dataset).filter(level=>dataset.observations.some(row=>row.indicator_id===indicatorId&&row.status==='observed'&&finite(row.value)&&areaFor(row.territory_id)?.level===level));
...
function populationPyramid() {
  ...
  const bands=record.bands||[],maximum=Math.max(1,...bands.flatMap(row=>[row.male,row.female])),width=760,center=380,half=320,rowHeight=24,height=70+bands.length*rowHeight;
...
function thematic() {
  ...
  const levels=comparisonLevelsFor(indicator.id);
...
function planning() {
  ...
  const outputs={docx:button('planning-docx','Download municipal planning diagnostic (Word)','','button'),markdown:button('planning-markdown','Download editable Markdown','','button'),html:button('planning-html','Print-ready HTML'),evidence_csv:button('planning-csv','Evidence CSV'),documents_csv:button('documents-csv','Materials and findings CSV')};
...
function normalizeLatestState(next) {
  const levels=comparisonLevelsFor(next.metric);
  if(page==='thematic'&&levels.length&&!levels.includes(next.level))next={...next,level:levels[0]};
...
app.addEventListener('click',async event=>{
  ...
  try {
    ...
    else if(action==='planning-docx')downloadBlob(planDocxBlob(dataset,state.selected,latestMode()?null:state.period),`${stem}-diagnostico-plan-desarrollo-municipal.docx`);

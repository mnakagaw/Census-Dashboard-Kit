import {
  ...
  periodsFor, observedPeriodsForArea, themePeriodCoverage, observationState, areaObservationState, localLevels, levelLabel, nationalOnly, initialState,
...
function metricCard(indicator) {
  ...
  const otherPeriods=result.value===null?observedPeriodsForArea(dataset,state.selected,indicator.id).filter(period=>period!==String(state.period)):[];
...
app.addEventListener('click',async event=>{
  ...
  try {
    ...
    else if(action==='use-period')commit({...state,period:target.dataset.period,metric:target.dataset.metric||state.metric,notices:[]});

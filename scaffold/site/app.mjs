...
const pageNames = {home:'Home',territorial:'Territorial diagnostic',thematic:'Thematic diagnostic',planning:'Development plan materials',database:'Data register'};
...
function territorial() {
  ...
  ${themes.map((theme,index)=>{const indicators=territorialIndicators.filter(indicator=>(indicator.theme||'Other')===theme);if(latestMode()){const coverage=themeLatestCoverage(dataset,state.selected,indicators),cards=`<div class="indicator-grid">${indicators.map(metricCard).join('')}</div>`;if(coverage.available)return `<section id="theme-${index}" class="theme-section"><h2 class="section-title">${e(theme)} <small>${e(currentArea().name)} · latest by item · ${coverage.available}/${coverage.total} acquired</small></h2>${cards}</section>`;return `<details id="theme-${index}" class="theme-section unavailable-theme"><summary><strong>${e(theme)}</strong><span>${e(currentArea().name)} · no acquired local value</span></summary><p class="missing-note">No acquired value for this selected area. Wider-area or national references, when present, are kept separate.</p>${cards}</details>`;}const coverage=themePeriodCoverage(dataset,state.selected,indicators,state.period),cards=`<div class="indicator-grid">${indicators.map(metricCard).join('')}</div>`;if(coverage.exact)return `<section id="theme-${index}" class="theme-section"><h2 class="section-title">${e(theme)} <small>${e(currentArea().name)} · ${e(state.period || 'No period')} · ${coverage.exact}/${coverage.total} acquired</small></h2>${cards}</section>`;const alternatives=coverage.otherPeriods.length?` · acquired: ${e(coverage.otherPeriods.join(', '))}`:'';const hint=coverage.otherPeriods.length?`Acquired for this area in ${e(coverage.otherPeriods.join(', '))}; choose a listed period inside to inspect it.`:'No acquired value for this selected area in any period. Wider-area or national references, when present, are kept separate.';return `<details id="theme-${index}" class="theme-section unavailable-theme"><summary><strong>${e(theme)}</strong><span>${e(currentArea().name)} · ${e(state.period || 'No period')} · 0/${coverage.total}${alternatives}</span></summary><p class="missing-note">${hint}</p>${cards}</details>`;}).join('')}
...
function planning() {
  ...
  const local=dataset.territories.filter(area=>area.level!=='national');
  const observed=dataset.indicators.filter(indicator=>{const period=latestMode()?latestDisplayPeriod(dataset,state.selected,indicator.id):state.period;return areaObservationState(dataset,state.selected,indicator.id,period).value!==null;}).length;
...
function database() {
  ...
  const censusStatus=dataset.analysis?.pilot?.primary_series_family==='census'
    ...
    : 'The current dataset may contain only initial national reference series. Official census and local sources must be integrated by the country adapter.';
  ...
  <section class="panel"><h2>Complete the country evidence in stages</h2><ol><li>${e(censusStatus)}</li><li>Acquire official census tables, census years, administrative codes and compatible boundaries for the legal planning level and needed internal-analysis levels.</li><li>Add sector and planning sources only after definitions, periods, populations and geographic correspondence have been verified.</li></ol></section>`;
}
function register() {
...
function normalizeLatestState(next) {
  ...
  const comparisonPeriod=latestComparisonPeriod(dataset,next.metric,next.level);
...
app.addEventListener('click',async event=>{
  ...
  try {
    ...
    else if(action==='planning-docx')downloadBlob(planDocxBlob(dataset,state.selected,latestMode()?null:state.period),`${stem}-development-plan.docx`);

import {evidenceRows, observedValue, safeUrl, statusLabel} from './model.mjs';
...
export function planDocxBytes(dataset,territoryId,period) {
  ...
  const body=[];
  body.push(paragraph(`${area.name} ${template.document_type}`,'Title',{keep:true}),paragraph(template.title,'Subtitle'),paragraph(`Editable working document for ${dataset.country.name}. Generated from the dashboard data edition ${dataset.generated_at}. It does not by itself establish consultation, approval or legal adoption.`));
  body.push(paragraph('Document basis','Heading1',{keep:true}),table([
    ['Field','Verified or selected value'],['Planning authority',template.authority],['Planning framework',settings.system?.label||'Not recorded'],['Planning cycle',settings.system?.cycle||'Not recorded'],['Area',`${area.name} · ${area.type} · ${area.official_code||'official code not recorded'}`],['Statistical evidence policy',period==null?'Latest confirmed value for each indicator; source year shown per row':`Selected period ${period}`],['Outline status',template.status==='verified_prescribed_index'?'Officially prescribed index verified':'Requirements-based outline; no fixed official index identified'],['Verification note',template.rationale]
  ],[2600,6500]));
  ...
  for(const group of sourceGroups){body.push(paragraph(group.label,'Heading2',{keep:true}),paragraph(group.note,'Caption'),...(group.sources.length?group.sources.map(source=>listItem(`${source.name} — ${source.url}; ${statusLabel(source.status)}; retrieved ${source.retrieved_at}`)):[paragraph('No source is registered in this category.') ]));}
  body.push(paragraph('Contents','Heading1',{keep:true}),...template.sections.map(section=>paragraph(`${section.number} ${section.title}`,'TOC1')));
  for(const section of template.sections) {
    body.push(paragraph(`${section.number} ${section.title}`,'Heading1',{pageBreak:true,keep:true}));
    body.push(paragraph(section.guidance));
    body.push(paragraph(`Legal basis: ${basisLabel(dataset,section.legal_basis)}`,'Caption'));
    body.push(paragraph(section.required?'Required content under the verified source.':'Optional content under the verified source.','Normal',{italic:true}));
    body.push(paragraph('[Prepare and review this section with the responsible authority. Keep facts, proposals, consultation records and formal decisions distinct.]'));
  }
  body.push(paragraph('Statistical evidence annex','Heading1',{pageBreak:true,keep:true}),paragraph(period==null?'Values below use the latest confirmed observation for each indicator in the selected area. Each row shows its source year. Missing values remain missing; national values are not copied into local areas.':'Values below use the selected area and period. Missing values remain missing; national values are not copied into local areas.'));
  body.push(table([['Indicator','Value','Unit','Year / period','Status','Source'],...evidence.map(({indicator,row,value,status,source,period:displayPeriod})=>[indicator.name,observedValue(row)===null?'—':String(value),row?.unit||indicator.unit,row?.period||displayPeriod||'Not recorded',statusLabel(status),source?.name||'No source acquired'])],[2500,1000,1000,1200,1400,2200]));
  ...
  body.push(paragraph('Evidence gaps and next actions','Heading1',{keep:true}),...(gaps.length?gaps.map(gap=>listItem(`${gap.category} — ${statusLabel(gap.status)}: ${gap.detail} Next: ${gap.next_action||'Verify with the responsible source.'}`)):[paragraph('No gaps are recorded. This is not a certification of complete evidence.') ]));
  body.push(paragraph('Document control','Heading1',{pageBreak:true,keep:true}),table([['Item','Value'],['Country and area',`${dataset.country.name} · ${area.name}`],['Dataset edition',dataset.generated_at],['Outline verified',template.verified_at],['Document status','Editable working document; consultation and approval must be recorded separately.']],[2600,6500]));
  ...
  const core=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><dc:title>${xml(`${area.name} ${template.document_type}`)}</dc:title><dc:subject>${xml(template.title)}</dc:subject><dc:creator>Census Dashboard Kit</dc:creator><cp:keywords>development plan; territorial evidence; ${xml(dataset.country.id)}</cp:keywords><dcterms:created xsi:type="dcterms:W3CDTF">${created}</dcterms:created><dcterms:modified xsi:type="dcterms:W3CDTF">${created}</dcterms:modified></cp:coreProperties>`;

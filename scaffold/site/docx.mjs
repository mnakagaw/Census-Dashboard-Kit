import {evidenceRows, observedValue, safeUrl, statusLabel, evidenceStatus} from './model.mjs';
...
function evidenceTable(rows) {
  return table([['Indicator','Latest confirmed value','Unit','Year / period','Status','Source'],...rows.map(({indicator,row,value,status,source,period})=>[indicator.name,observedValue(row)===null?'—':String(value),row?.unit||indicator.unit,row?.period||period||'Not recorded',evidenceStatus(indicator,row,status),source?.name||'No source acquired'])],[2500,1200,900,1200,1400,2200]);
}
function evidenceStatement(entry,areaName) {
  const {indicator,row,value,source,period}=entry,observed=observedValue(row);
  if(observed===null)return `${indicator.name}: no confirmed value is recorded for ${areaName}.`;
  const unit=row?.unit||indicator.unit||'',displayPeriod=row?.period||period||'period not recorded';
  return `${indicator.name}: ${areaName} recorded ${value}${unit?` ${unit}`:''} in ${displayPeriod}. Source: ${source?.name||'source not acquired'}.`;
}
...
export function planDocxBytes(dataset,territoryId,period) {
  ...
  for(const entry of evidence){const theme=entry.indicator.theme||'Other';if(!byTheme.has(theme))byTheme.set(theme,[]);byTheme.get(theme).push(entry);}
  const body=[];
  body.push(paragraph(`Diagnóstico para el Plan de Desarrollo Municipal — ${area.name}`,'Title',{keep:true}),paragraph(`Territorial evidence organized with the planning index applicable to ${dataset.country.name}`,'Subtitle'),paragraph(`This municipal development planning diagnostic assembles the selected area's confirmed statistics, source years, planning references and evidence gaps for local analysis and consultation.`));
  body.push(paragraph('Document basis','Heading1',{keep:true}),table([
    ['Field','Source-checked or selected value'],['Planning reference',template.authority],['Planning framework',settings.system?.label||'Not recorded'],['Planning cycle',settings.system?.cycle||'Not recorded'],['Area',`${area.name} · ${area.type} · ${area.official_code||'administrative code not recorded'}`],['Statistical evidence policy',period==null?'Latest confirmed value for each indicator; source year shown per row':`Selected period ${period}`],['Index basis',template.status==='verified_prescribed_index'?'Prescribed index verified in the cited source':'Diagnostic structure aligned to the cited governance and planning sources']
  ],[2600,6500]));
  ...
  body.push(paragraph('Index based on the applicable law or guidance','Heading1',{keep:true}),...template.sections.map(section=>paragraph(`${section.number} ${section.title}`,'TOC1')));
  body.push(paragraph('Diagnóstico Territorial','Heading1',{pageBreak:true,keep:true}),paragraph(period==null?'Each theme below uses the latest confirmed value available for every indicator in the selected area and shows its source year. Missing values remain missing.':'Each theme below uses the selected period. Missing values remain missing.'));
  ...
  body.push(paragraph('Document control','Heading1',{pageBreak:true,keep:true}),table([['Item','Value'],['Country and area',`${dataset.country.name} · ${area.name}`],['Dataset edition',dataset.generated_at],['Index basis checked',template.verified_at],['Document type','Diagnóstico para el Plan de Desarrollo Municipal']],[2600,6500]));
  ...
  const core=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><dc:title>${xml(`Diagnóstico para el Plan de Desarrollo Municipal — ${area.name}`)}</dc:title><dc:subject>${xml(`Territorial evidence organized with ${template.title}`)}</dc:subject><dc:creator>Census Dashboard Kit</dc:creator><cp:keywords>municipal development planning diagnostic; territorial evidence; ${xml(dataset.country.id)}</cp:keywords><dcterms:created xsi:type="dcterms:W3CDTF">${created}</dcterms:created><dcterms:modified xsi:type="dcterms:W3CDTF">${created}</dcterms:modified></cp:coreProperties>`;

...
export function planDocxBytes(dataset,territoryId,period) {
  ...
  body.push(paragraph('Statistical evidence annex','Heading1',{pageBreak:true,keep:true}),paragraph('Values below use the selected area and period. Missing values remain missing; national values are not copied into local areas.'));
  body.push(table([['Indicator','Value','Unit','Status','Source'],...evidence.map(({indicator,row,value,status,source})=>[indicator.name,observedValue(row)===null?'—':String(value),row?.unit||indicator.unit,statusLabel(status),source?.name||'No source acquired'])],[3000,1200,1200,1600,2500]));

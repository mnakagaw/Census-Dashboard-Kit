...
> [!NOTE]
> 1.0系で作られたラオス試作のように、人口ファイル一つと全国WDIだけを入れ、取得済みExcelの大半を使わず、境界のない大きな地図と「データなし」を並べたものは完成品ではありません。1.3.0では、取得資料の全表・数値列、6分野の探索、各選択階層の境界、別年データへの導線、全国参考値の分離、表示言語を納品ゲートで確認します。詳しくは[汎用AIの失敗例と防止確認](docs/AI_FAILURE_MODE_REVIEW.md)を参照してください。
...
取得したExcelの全シート、CSV/APIの全フィールド、PDFの目次・付表・継続表を確認し、全数値列をevidence/SOURCE_TABLE_INVENTORY.jsonへ棚卸ししてください。採用・不採用と具体的理由、原本の再配布可否を残し、利用可能候補を見つけたまま未統合で納品しないでください。再配布不可・要許諾の原本をPublic成果へ含めないでください。6分野の結果はevidence/THEME_COVERAGE.jsonへ記録してください。
...
地方観測が一件でもある場合、初期表示は地方比較できる指標・年にしてください。選択年に値がなくても同じ地域・指標の別年を取得済みなら、取得年と切替ボタンを表示してください。全国系列しかないテーマや選択年に全件欠測のテーマは折りたたみ、空の地域地図・凡例・全件表を反復せず、短い不足説明と取得済み指標への導線を示してください。全国系列は地方値と混ぜず、全国参考として分離してください。
...
完全な地域、データの少ない地域、特殊な行政区分を代表例として、実際のブラウザーで確認してください。最後に「この国には本当に他の地方データがないか」「取得済み資料に未採用の有用な数値列が残っていないか」「別年データをデータなしに見せていないか」を反証確認してください。

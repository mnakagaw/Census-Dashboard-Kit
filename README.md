...
地方観測が一件でもある場合、初期表示は地方比較できる指標にしてください。地域診断・計画資料では、各テーマ・指標について選択地域で確認できた最新版を自動表示し、値の横に出典年を付けてください。年セレクター、過去年への切替、時系列グラフ、履歴CSVは国別3ページに置きません。テーマ診断の地域間比較は、その指標・階層で利用できる最も新しい共通年を自動選択し、順位・地図・中央値・比較CSVを同年に揃えてください。全国系列しかないテーマや地方値が全件欠測のテーマは折りたたみ、空の地域地図・凡例・全件表を反復せず、短い不足説明と取得済み指標への導線を示してください。全国系列は地方値と混ぜず、全国参考として分離してください。詳しくは[最新版表示方針](docs/LATEST_VALUE_POLICY.md)に従ってください。

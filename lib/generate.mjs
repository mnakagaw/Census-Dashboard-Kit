...
export async function generateSite({dataset, outDir}) {
  ...
  const lines=[
    ...
    '## Available outputs', '',
    `Adopted planning outputs: ${planningSettings(dataset).outputs.join(', ') || 'None; use original references and the country workflow'}. A completed country delivery must include docx and either a verified country document_template or an explicitly provisional evidence outline with a constrained planning-system record. The Word output may use an officially prescribed index, a cited requirements-based outline only when the planning authority prescribes no fixed index, or a clearly labelled provisional evidence outline that makes no legal-applicability claim. Markdown and print-ready HTML remain working evidence outputs.`,

...
const pages = {home:'Home', territorial:'Territorial diagnostic', thematic:'Thematic diagnostic', planning:'Development plan materials', database:'Data register'};
const primaryPages = new Set(['home', 'territorial', 'thematic', 'planning']);
...
export function pageShell({country, page='home'}) {
  ...
  const links=Object.entries(pages).filter(([key])=>primaryPages.has(key)).map(([key,label])=>`<a href="${prefix}${key==='home'?'':key+'/'}" data-page-link="${key}" data-i18n="${escapeHtml(label)}"${key===page?' aria-current="page"':''}>${escapeHtml(label)}</a>`).join('');
...
export async function generateSite({dataset, outDir}) {
  ...
  const lines=[
    ...
    'The home, territorial/, thematic/ and planning/ entry pages share the same local assets and dataset. The supporting database/ register exposes definitions and source records. Links and fetch URLs work at the domain root or below a hosting subdirectory.',

...
const WORLD_REGISTRY_PATH = 'config/world-country-area-registry.json';
const WORLD_SOURCE_PATH = 'config/world-source-preflight.json';
...
const AREADATA_FEEDBACK_PATH = 'config/areadata-source-feedback.json';
...
function validatePrioritySourcePreflight(preflight, priorityRegistry) {
  ...
  for (const record of preflight.records) {
    ...
    validateUnsdCensusSelections(census, record.iso3);
...
function validateLinkList(list, prefix) {
  invariant(Array.isArray(list), `${prefix} must be an array`);
  for (const [index, item] of list.entries()) {
    invariant(item?.title && item?.url, `${prefix}[${index}] metadata missing`);
    publicWebUrl(item.url, `${prefix}[${index}].url`);
  }
}

function validateUnsdCensusSelections(census, prefix) {
  invariant(Object.hasOwn(census, 'latest_un_census_listing')
    && Object.hasOwn(census, 'latest_un_census_linked_listing'),
  `${prefix} UNSD latest/listed-link census separation missing`);
  const validateSelection = (selection, field, linkedOnly) => {
    if (selection === null) return;
    invariant([1990, 2000, 2010, 2020, 2030].includes(selection?.round)
      && /^\d{4}-\d{4}$/.test(selection?.round_period || '')
      && selection?.date_text && selection?.country_label && Array.isArray(selection?.links),
    `${prefix}.${field} metadata missing`);
    for (const [index, url] of selection.links.entries()) publicWebUrl(url, `${prefix}.${field}.links[${index}]`);
    if (linkedOnly) {
      invariant(selection.links.length > 0 && selection.primary_url === selection.links[0]
        && selection.link_status === 'unsd_link_listed', `${prefix}.${field} must identify an UNSD-linked listing`);
    } else if (selection.links.length) {
      invariant(selection.primary_url === selection.links[0] && selection.link_status === 'unsd_link_listed',
        `${prefix}.${field} linked metadata inconsistent`);
    } else {
      invariant(selection.primary_url === null && selection.link_status === 'no_unsd_link_listed',
        `${prefix}.${field} unlinked metadata inconsistent`);
    }
  };
  validateSelection(census.latest_un_census_listing, 'latest_un_census_listing', false);
  validateSelection(census.latest_un_census_linked_listing, 'latest_un_census_linked_listing', true);
  if (census.latest_un_census_listing && census.latest_un_census_linked_listing) {
    invariant(census.latest_un_census_linked_listing.round <= census.latest_un_census_listing.round,
      `${prefix} linked UNSD listing cannot be newer than the latest UNSD listing`);
  }
}

function validateWorldRegistry(registry) {
  invariant(registry?.schema_version && registry?.as_of && registry?.definition && registry?.use,
    'world country/area registry metadata missing');
  invariant(registry.counts?.un_m49_countries_and_areas === 248
    && registry.counts?.supplemental_operational_identities === 2
    && registry.counts?.total === 250, 'world registry counts must be 248 UN M49 plus two supplements');
  invariant(Array.isArray(registry.sources) && registry.sources.length >= 2, 'world registry sources missing');
  for (const source of registry.sources) publicWebUrl(source.url, `world registry source ${source.id}`);
  invariant(Array.isArray(registry.countries_and_areas) && registry.countries_and_areas.length === 250,
    'world registry must contain 250 countries/areas');
  const iso3s = new Set(), iso2s = new Set(), m49s = new Set();
  let m49 = 0, supplemental = 0;
  for (const area of registry.countries_and_areas) {
    invariant(/^[A-Z]{3}$/.test(area?.iso3) && /^[A-Z]{2}$/.test(area?.iso2), 'world registry ISO identity invalid');
    invariant(area.name_en && Array.isArray(area.aliases) && area.registry_basis && area.classification_warning,
      `${area.iso3} world identity metadata missing`);
    invariant(!iso3s.has(area.iso3) && !iso2s.has(area.iso2), `duplicate world identity ${area.iso3}/${area.iso2}`);
    iso3s.add(area.iso3); iso2s.add(area.iso2);
    if (area.registry_basis === 'un_m49_country_or_area') {
      invariant(/^\d{3}$/.test(area.m49), `${area.iso3} UN M49 code invalid`);
      invariant(!m49s.has(area.m49), `duplicate UN M49 code ${area.m49}`); m49s.add(area.m49); m49++;
    } else {
      invariant(['TWN', 'XKX'].includes(area.iso3) && area.m49 === null,
        'only TWN and XKX may be explicit non-M49 supplements');
      if (area.iso3 === 'TWN') invariant(area.iso_numeric === '158', 'TWN ISO numeric identity missing');
      supplemental++;
    }
    invariant(area.global_region?.code === '001', `${area.iso3} world region missing`);
    invariant((area.region?.code && area.subregion?.code)
      || (area.iso3 === 'ATA' && area.region?.code === null && area.subregion?.code === null),
    `${area.iso3} M49 region hierarchy missing without a documented structural exception`);
  }
  invariant(m49 === 248 && supplemental === 2 && iso3s.has('ESP') && iso3s.has('FIN')
    && iso3s.has('TWN') && iso3s.has('XKX'),
    'world registry identity coverage incomplete');
}

function validateWorldSourcePreflight(preflight, worldRegistry) {
  invariant(preflight?.schema_version && preflight?.as_of && preflight?.meaning && Array.isArray(preflight.records),
    'world source preflight metadata missing');
  invariant(preflight.records.length === 250, 'world source preflight must contain 250 records');
  const worldIds = new Set(worldRegistry.countries_and_areas.map(area => area.iso3));
  const seen = new Set();
  for (const record of preflight.records) {
    invariant(worldIds.has(record?.iso3) && !seen.has(record.iso3), `invalid or duplicate world source preflight ${record?.iso3}`);
    seen.add(record.iso3);
    invariant(record.iso2 && record.name_en && record.checked_at && record.world_identity?.classification_warning,
      `${record.iso3} world source identity missing`);
    const census = record.national_statistics_and_census;
    invariant(census?.status && census?.national_statistics_office?.url && census?.national_statistics_office?.authority_relation
      && census?.un_census_dates_source && Array.isArray(census.un_census_rounds) && census.required_next_step,
    `${record.iso3} world census/statistics entrypoints missing`);
    validateUnsdCensusSelections(census, `${record.iso3}.world`);
    publicWebUrl(census.national_statistics_office.url, `${record.iso3}.world.national_statistics_office.url`);
    httpsUrl(census.un_census_dates_source, `${record.iso3}.world.un_census_dates_source`);
    validateLinkList(census.country_specific_entrypoints, `${record.iso3}.world.statistics_and_census`);
    const planning = record.planning_law_and_materials;
    invariant(planning?.status && planning?.legal_catalogue && planning?.required_next_step,
      `${record.iso3} world planning/legal entrypoints missing`);
    httpsUrl(planning.legal_catalogue, `${record.iso3}.world.legal_catalogue`);
    validateLinkList(planning.official_country_entrypoints, `${record.iso3}.world.planning_and_law`);
    validateLinkList(planning.regional_candidates, `${record.iso3}.world.planning_regional_candidates`);
    const geography = record.geography_and_codes;
    invariant(geography?.status && geography?.geoboundaries_adm1_api && geography?.geoboundaries_adm2_api
      && geography?.un_salb_catalogue && geography?.hdx_targeted_search && geography?.required_next_step,
    `${record.iso3} world geography entrypoints missing`);
    for (const [key, value] of Object.entries(geography)) {
      if (key.endsWith('_api') || key.endsWith('_catalogue') || key.endsWith('_search')) httpsUrl(value, `${record.iso3}.world.${key}`);
    }
    validateLinkList(geography.country_specific_entrypoints, `${record.iso3}.world.geography_and_codes`);
    validateLinkList(geography.regional_candidates, `${record.iso3}.world.geography_regional_candidates`);
    const international = record.international_data_candidates;
    invariant(international?.status && international?.world_bank_country_api && international?.hdx_country_search
      && international?.required_next_step, `${record.iso3} world international candidates missing`);
    for (const [key, value] of Object.entries(international)) {
      if (typeof value === 'string' && !['status', 'required_next_step'].includes(key)) httpsUrl(value, `${record.iso3}.world.${key}`);
    }
    validateLinkList(international.regional_candidates, `${record.iso3}.world.international_regional_candidates`);
    invariant(Array.isArray(record.anti_shortcut_rules) && record.anti_shortcut_rules.length >= 7,
      `${record.iso3} world anti-shortcut rules missing`);
  }
  invariant(seen.size === worldIds.size && [...worldIds].every(id => seen.has(id)),
    'world source preflight does not cover the complete world registry');
  for (const iso3 of ['ESP', 'FIN', 'TWN']) {
    const record = preflight.records.find(item => item.iso3 === iso3);
    invariant(record.national_statistics_and_census.country_specific_entrypoints.length >= 4,
      `${iso3} detailed country census/statistics pack missing`);
    invariant(record.planning_law_and_materials.official_country_entrypoints.length >= 2,
      `${iso3} detailed country planning/law pack missing`);
    invariant(record.geography_and_codes.country_specific_entrypoints.length >= 3,
      `${iso3} detailed country geography/code pack missing`);
  }
}

function validateAreaDataSourceFeedback(registry, worldRegistry) {
  invariant(registry?.schema_version === '1.0' && registry?.as_of && registry?.purpose && registry?.status_boundary,
    'AreaData source-feedback registry metadata missing');
  invariant(Array.isArray(registry.records), 'AreaData source-feedback records missing');
  const worldIds = new Set(worldRegistry.countries_and_areas.map(area => area.iso3));
  const feedbackIds = new Set(), identities = new Set();
  ...
  for (const record of registry.records) {
    invariant(/^AREADATA_[A-Z]{3}_[0-9A-F]{16}$/.test(record?.feedback_id), 'AreaData feedback_id invalid');
    invariant(!feedbackIds.has(record.feedback_id), `duplicate AreaData feedback ${record.feedback_id}`); feedbackIds.add(record.feedback_id);
    invariant(worldIds.has(record.iso3), `${record.feedback_id} uses an unknown world identity`);
    invariant(/^[A-Z0-9][A-Z0-9_.-]+$/.test(record.source_id || '') && record.role && record.title && record.publisher,
      `${record.feedback_id} source identity missing`);
    publicWebUrl(record.url, `${record.feedback_id}.url`);
    if (record.superseded_by) invariant(/^AREADATA_[A-Z]{3}_[0-9A-F]{16}$/.test(record.superseded_by),
      `${record.feedback_id} superseded_by invalid`);
    if (record.supersedes_feedback_id) invariant(/^AREADATA_[A-Z]{3}_[0-9A-F]{16}$/.test(record.supersedes_feedback_id),
      `${record.feedback_id} supersedes_feedback_id invalid`);
    const identity = `${record.iso3}|${record.role}|${record.url}`;
    invariant(!identities.has(identity), `duplicate AreaData country/role/URL ${identity}`); identities.add(identity);
    invariant(record.authority_type && stages.has(record.evidence_stage), `${record.feedback_id} authority or evidence stage invalid`);
    invariant(/^\d{4}-\d{2}-\d{2}$/.test(record.first_seen_at || '')
      && /^\d{4}-\d{2}-\d{2}$/.test(record.last_verified_at || '')
      && record.first_seen_at <= record.last_verified_at, `${record.feedback_id} date range invalid`);
    invariant(Array.isArray(record.geographic_levels) && Array.isArray(record.reference_periods) && Array.isArray(record.formats)
      && record.reuse_note && record.current_project_evidence_status === 'not_acquired_by_kit_preflight',
    `${record.feedback_id} reusable-source boundary missing`);
    invariant(Array.isArray(record.origin_events) && record.origin_events.length, `${record.feedback_id} origin evidence missing`);
    for (const event of record.origin_events) {
      invariant(/^[0-9a-f]{40}$/.test(event?.origin_commit || '') && event.exported_at && event.evidence_path
        && stages.has(event.evidence_stage), `${record.feedback_id} origin event invalid`);
      invariant(!path.isAbsolute(event.evidence_path) && !event.evidence_path.replaceAll('\\', '/').split('/').includes('..'),
        `${record.feedback_id} origin evidence path must be repository-relative`);
      invariant(event.artifact_sha256 === null || /^[0-9a-f]{64}$/.test(event.artifact_sha256 || ''),
        `${record.feedback_id} origin artifact hash invalid`);
    }
    invariant(!Object.hasOwn(record, 'observations') && !Object.hasOwn(record, 'data'),
      `${record.feedback_id} feedback must not embed observations`);
  }
  for (const record of registry.records) {
    if (record.superseded_by) invariant(registry.records.some(other => other.feedback_id === record.superseded_by
      && other.supersedes_feedback_id === record.feedback_id), `${record.feedback_id} correction link missing`);
  }
}

export async function loadSourceCatalog(root = defaultRoot) {
  const base = path.resolve(root);
  const [common, registry, worldRegistry, worldSourcePreflight, jicaPriority, prioritySourcePreflight, americasRecipes, areaDataFeedback] = await Promise.all([
    readJson(base, COMMON_PATH), readJson(base, COUNTRY_PATH), readJson(base, WORLD_REGISTRY_PATH),
    readJson(base, WORLD_SOURCE_PATH), readJson(base, JICA_PRIORITY_PATH), readJson(base, JICA_PRIORITY_SOURCE_PATH),
    readJson(base, AMERICAS_RECIPE_PATH), readJson(base, AREADATA_FEEDBACK_PATH),
  ]);
  validateCommon(common);
  validateWorldRegistry(worldRegistry);
  validateWorldSourcePreflight(worldSourcePreflight, worldRegistry);
  ...
  validateAreaDataSourceFeedback(areaDataFeedback, worldRegistry);
  ...
  return {
    version: '1.2',
    as_of: [common.as_of, registry.as_of, worldRegistry.as_of, worldSourcePreflight.as_of, jicaPriority.as_of, americasRecipes.as_of, areaDataFeedback.as_of].sort().at(-1),
    common_sources: common.sources,
    countries,
    world_countries_and_areas: worldRegistry.countries_and_areas,
    world_source_records: worldSourcePreflight.records,
    ...
    areadata_source_feedback: areaDataFeedback.records.filter(record => !record.superseded_by),
    areadata_source_feedback_registry: {
      as_of: areaDataFeedback.as_of,
      purpose: areaDataFeedback.purpose,
      status_boundary: areaDataFeedback.status_boundary,
    },
    ...
    world_registry: {
      as_of: worldRegistry.as_of,
      definition: worldRegistry.definition,
      use: worldRegistry.use,
      counts: worldRegistry.counts,
      sources: worldRegistry.sources,
      classification_warning: worldRegistry.classification_warning,
    },
    ...
    coverage: {
      ...
      world_countries_and_areas: worldRegistry.countries_and_areas.length,
      world_source_preflights: worldSourcePreflight.records.length,
      un_m49_countries_and_areas: worldRegistry.counts.un_m49_countries_and_areas,
      ...
      areadata_feedback_sources: areaDataFeedback.records.filter(record => !record.superseded_by).length,
...
const localizedCountryNameCache = new WeakMap();

function localizedCountryNames(area) {
  if (localizedCountryNameCache.has(area)) return localizedCountryNameCache.get(area);
  const names = [];
  for (const locale of ['en', 'ja', 'es', 'fr']) {
    try {
      const display = new Intl.DisplayNames([locale], { type: 'region', fallback: 'none' });
      const value = display.of(area.iso2);
      if (value) names.push(value);
    } catch { /* Runtime locale data can vary; stored names and codes remain authoritative. */ }
  }
  localizedCountryNameCache.set(area, names);
  return names;
}

export function findWorldCountryRecord(catalog, requested) {
  const query = normal(requested);
  if (!query) return null;
  const matches = catalog.world_countries_and_areas.filter(area => [
    area.iso3, area.iso2, area.m49, area.iso_numeric, area.name_en, ...(area.aliases || []), ...localizedCountryNames(area),
  ].some(value => normal(value) === query));
  if (matches.length > 1) throw new Error(`Ambiguous world country/area identity: ${requested}`);
  return matches[0] || null;
}

export function findWorldSourceRecord(catalog, requested) {
  const world = findWorldCountryRecord(catalog, requested);
  return world ? catalog.world_source_records.find(record => record.iso3 === world.iso3) || null : null;
}

export function findAreaDataSourceFeedback(catalog, requested) {
  const world = findWorldCountryRecord(catalog, requested);
  return world ? catalog.areadata_source_feedback.filter(record => record.iso3 === world.iso3) : [];
}
...
export function buildSourcePreflight(catalog, country, { theme } = {}) {
  ...
  const world = findWorldCountryRecord(catalog, country.id);
  const worldSources = findWorldSourceRecord(catalog, country.id);
  ...
  const areaDataFeedback = findAreaDataSourceFeedback(catalog, country.id);
  ...
  return {
    ...
    country: { iso3: country.id, name: country.name || record?.names?.[0] || world?.name_en || country.id },
    world_context: world ? {
      status: 'world_country_or_area_registry_match',
      registry_as_of: catalog.world_registry.as_of,
      registry_basis: world.registry_basis,
      iso2: world.iso2,
      m49: world.m49,
      region: world.region,
      subregion: world.subregion,
      intermediate_region: world.intermediate_region,
      classification_warning: world.classification_warning,
    } : {
      status: 'outside_world_country_or_area_registry',
      registry_as_of: catalog.world_registry.as_of,
    },
    world_source_preflight: worldSources,
    ...
    areadata_source_feedback: {
      status: areaDataFeedback.length ? 'reusable_discovery_sources_available' : 'no_areadata_feedback_registered',
      registry_as_of: catalog.areadata_source_feedback_registry.as_of,
      status_boundary: catalog.areadata_source_feedback_registry.status_boundary,
      sources: areaDataFeedback.map(source => ({ ...source, current_project_evidence_status: 'not_acquired_by_kit_preflight' })),
    },
    ...
    required_actions: [
      ...
      areaDataFeedback.length
        ? 'Review the AreaData feedback sources before generic search, then refresh their URLs, acquire permitted originals and create new project-specific receipts before adoption.'
        : 'No AreaData source feedback is registered for this country; return newly verified public official source locations through the AreaData feedback contract when found.',
      researched
      ...
      worldSources?.national_statistics_and_census?.country_specific_entrypoints?.length
        || worldSources?.planning_law_and_materials?.official_country_entrypoints?.length
        || worldSources?.geography_and_codes?.country_specific_entrypoints?.length
        ? 'Open every country-specific official entrypoint in the world preflight first; confirm current editions and inventory resources instead of restarting from a generic web search.'
        : 'Use the authority and catalogue addresses in the world preflight to locate the current country-specific official census, legal-planning and administrative-geography resources.',
    ...
    summary: {
      ...
      world_registry_match: Boolean(world),
      world_source_address_categories: worldSources ? 4 : 0,
      world_country_specific_entrypoints: worldSources ? [
        ...(worldSources.national_statistics_and_census.country_specific_entrypoints || []),
        ...(worldSources.planning_law_and_materials.official_country_entrypoints || []),
        ...(worldSources.geography_and_codes.country_specific_entrypoints || []),
      ].length : 0,
      ...
      areadata_feedback_sources: areaDataFeedback.length,
...
function renderUnsdCensusSelection(selection) {
  if (!selection) return 'none recorded';
  const date = selection.primary_url ? `[${md(selection.date_text)}](${selection.primary_url})` : md(selection.date_text);
  return `${selection.round} round / ${date} / ${selection.primary_url ? 'UNSD link listed' : 'no UNSD link'}`;
}
...
export function renderSourcePreflightMarkdown(preflight) {
  ...
  const lines = [
    ...
    '',
    '## World identity and source-address preflight',
    '',
  ];
  const world = preflight.world_context;
  const worldAddress = preflight.world_source_preflight;
  if (world?.status === 'world_country_or_area_registry_match' && worldAddress) {
    const census = worldAddress.national_statistics_and_census;
    const planning = worldAddress.planning_law_and_materials;
    const geography = worldAddress.geography_and_codes;
    const international = worldAddress.international_data_candidates;
    lines.push(
      `UN M49 identity: **${md(preflight.country.name)}**; ISO3 **${preflight.country.iso3}**; ISO2 **${md(world.iso2)}**; M49 **${md(world.m49 ?? 'supplemental identity')}**.`,
      '',
      `Region: ${md(world.region?.name || 'Not assigned by UN M49')} / ${md(world.subregion?.name || 'Not assigned by UN M49')}${world.intermediate_region?.name ? ` / ${md(world.intermediate_region.name)}` : ''}.`,
      '',
      `Classification boundary: ${md(world.classification_warning)}`,
      '',
      `Checked: **${md(worldAddress.checked_at)}**. These links are starting addresses only; they are not acquired evidence.`,
      '',
      `- Statistics/census authority: [${md(census.national_statistics_office.agency)}](${census.national_statistics_office.url}) (${md(census.national_statistics_office.authority_relation)})`,
      `- Census-round directory: [UNSD census dates](${census.un_census_dates_source}); ${census.un_census_rounds.length} matching row(s) recorded`,
      `- Latest completed census listed by UNSD: ${renderUnsdCensusSelection(census.latest_un_census_listing)}`,
      `- Latest completed listing with an UNSD link: ${renderUnsdCensusSelection(census.latest_un_census_linked_listing)}`,
      `- Legal catalogue: [FAOLEX country profile](${planning.legal_catalogue})`,
      `- Boundary/code candidates: [geoBoundaries ADM1](${geography.geoboundaries_adm1_api}); [ADM2](${geography.geoboundaries_adm2_api}); [UN SALB](${geography.un_salb_catalogue}); [HDX search](${geography.hdx_targeted_search})`,
      `- International-data candidates: [World Bank country query](${international.world_bank_country_api}); [HDX country search](${international.hdx_country_search})`,
      '',
    );
    const groups = [
      ['Country-specific statistics and census entrypoints', census.country_specific_entrypoints],
      ['Country-specific geography and code entrypoints', geography.country_specific_entrypoints],
      ['Country-specific planning and law entrypoints', planning.official_country_entrypoints],
      ['Regional statistical candidates', international.regional_candidates],
      ['Regional geography candidates', geography.regional_candidates],
      ['Regional legal/planning candidates', planning.regional_candidates],
    ];
    for (const [title, entries] of groups) {
      if (!entries?.length) continue;
      lines.push(`### ${title}`, '');
      for (const entry of entries) lines.push(`- [${md(entry.title)}](${entry.url})${entry.note ? ` — ${md(entry.note)}` : ''}`);
      lines.push('');
    }
    lines.push('The agent must open the relevant official pages, identify actual downloadable/API resources, record retrieval receipts and hashes, inspect tables and fields, and verify geographic compatibility before adopting any evidence.', '');
  } else {
    lines.push('No world registry match is available. Resolve the country/area identity before collection.', '');
  }
  lines.push(
    '## Priority context',
    '',
  ...
  const feedback = preflight.areadata_source_feedback;
  lines.push('', '## AreaData source feedback', '');
  if (feedback.sources.length) {
    lines.push(
      `${md(feedback.status_boundary)}`,
      '',
      '| Role | Source | AreaData evidence stage | Last verified | Reuse note |',
      '|---|---|---|---|---|',
    );
    for (const source of feedback.sources) {
      lines.push(`| ${md(source.role)} | [${md(source.title)}](${source.url}) — ${md(source.publisher)} | ${md(source.evidence_stage)} | ${md(source.last_verified_at)} | ${md(source.reuse_note)} |`);
    }
    lines.push('', 'Refresh each URL and create new project-specific acquisition, content and geography evidence before using any value.');
  } else {
    lines.push('No reviewed AreaData feedback source is registered for this country. Newly verified public official locations should be returned through the AreaData feedback bundle after the country work.');
  }
  lines.push(
    '',
  ...
  if (address) {
    const census = address.national_statistics_and_census;
    const nso = census.national_statistics_office;
    ...
    lines.push(
      ...
      `- UN census-round record: [UNSD census dates](${census.un_census_dates_source}); ${census.un_census_rounds.length} matching row(s) recorded`,
      `- Latest completed census listed by UNSD: ${renderUnsdCensusSelection(census.latest_un_census_listing)}`,
      `- Latest completed listing with an UNSD link: ${renderUnsdCensusSelection(census.latest_un_census_linked_listing)}`,
    ...
    lines.push('No JICA-specific supplement applies. Use the world source-address preflight above and complete the same acquisition and verification work.');
  }

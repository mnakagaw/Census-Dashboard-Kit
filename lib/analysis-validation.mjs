    ...
    if(!object(config.aggregation))fail('analysis.aggregation must be an object');
      ...
      for(const [index,rule] of array(config.aggregation.rules).entries()) {
        ...
        if(rule.method==='weighted_mean') {
          if(data.indicators.find(item=>item.id===rule.indicator_id)?.aggregation!=='weighted_rate')fail(`${path} requires indicators[].aggregation=weighted_rate`);
          if(!text(rule.weight_indicator_id) || !array(data.indicators).some(item=>item.id===rule.weight_indicator_id))fail(`${path}.weight_indicator_id must identify a known weight indicator`);
        }
        if(rule.method==='ratio')warnings.push(`${path} is declared but this runtime does not calculate it without compatible numerator and denominator component series; it remains unavailable.`);
      }
    }
  }

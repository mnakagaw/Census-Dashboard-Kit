...
export async function collectCountry({ country, requestedCountry = country, identity = null, rawDir, fetchImpl = globalThis.fetch, onProgress = () => {} }) {
  ...
  let economy, worldBankDirectoryMatched = true;
  try { economy = resolveCountryName(country, countries.records); }
  catch (error) {
    const validIdentity = identity && identity.iso3 === String(country).trim().toUpperCase()
      && /^[A-Z]{3}$/.test(identity.iso3) && /^[A-Z]{2}$/.test(identity.iso2) && typeof identity.name === 'string' && identity.name.trim();
    if (!validIdentity || !/not resolved/.test(error.message)) throw error;
    economy = { id: identity.iso3, iso2Code: identity.iso2, name: identity.name,
      region: { id: 'JICA', value: 'JICA priority source preflight' } };
    worldBankDirectoryMatched = false;
    onProgress(`World Bank directory has no individual economy record for ${identity.iso3}; preserving the verified UN M49/JICA identity and continuing source research.`);
  }
  ...
  const directorySource = { id: 'world-bank-countries', name: 'World Bank economy directory', url: countriesUrl.href,
    publisher: 'World Bank', reference_period: 'Current directory at collection time', status: worldBankDirectoryMatched ? 'ready' : 'unavailable', ...receiptFields(countries.receipts),
    license: WB_LICENSE, note: worldBankDirectoryMatched
      ? 'Individual World Bank economies only; regional and income aggregates excluded. Economy codes and names do not assert sovereign or administrative status.'
      : `No World Bank individual-economy directory record matched ${economy.id}. Identity was retained from the source-backed JICA/UN M49 registry; WDI failures are not converted to zeros.` };
  const dataset = {
    schema_version: '0.2', generated_at: new Date().toISOString(),
    country: { id: economy.id, iso2: economy.iso2Code, name: economy.name, requested_name: String(requestedCountry).trim(), locale: 'en', national_territory_id: economy.id,
    ...
    gaps: [...series.flatMap((item) => item.gaps), ...geography.gaps,
      ...(!worldBankDirectoryMatched ? [{ category: 'national_international_series', source_id: directorySource.id, status: 'unavailable', detail: `${economy.id} is not an individual economy in the collected World Bank directory.`, next_action: 'Use the preflight national statistics office and other verified sources; keep missing WDI values explicit.' }] : []),

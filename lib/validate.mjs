...
export function validateDataset(data) {
  ...
  for (const i of indicators.values()) {
    ...
    if (i.display_role != null && !['primary','context','supplementary','calculation_input'].includes(i.display_role)) fail(`Invalid display_role for ${i.id}`);
  }
  ...
  for (const s of sources.values()) {
    ...
    if (s.artifact_alias_of != null && !sources.has(s.artifact_alias_of)) fail(`Source ${s.id} has unknown artifact_alias_of ${s.artifact_alias_of}`);
    if (isRawEvidencePath(s.raw_path) && /^[a-f0-9]{64}$/i.test(s.sha256 ?? '')) {
      const artifactKey = `${s.raw_path.trim().replaceAll('\\\\','/').toLowerCase()}|${s.sha256.toLowerCase()}`;
      const previous = sourceArtifacts.get(artifactKey);
      if (previous && s.artifact_alias_of !== previous.id && previous.artifact_alias_of !== s.id) {
        fail(`Sources ${previous.id} and ${s.id} claim the same raw artifact and SHA-256; use one source record or declare artifact_alias_of after reviewing the identity`);
      } else if (!previous) sourceArtifacts.set(artifactKey, s);
    }
  ...
  if (data.population_pyramids != null) {
    if (!Array.isArray(data.population_pyramids)) fail('population_pyramids must be an array when supplied');
    else {
      const pyramidKeys=new Set();
      for(const [index,pyramid] of data.population_pyramids.entries()) {
        const label=`population_pyramids[${index}]`;
        if(!pyramid || typeof pyramid!=='object' || Array.isArray(pyramid)){fail(`${label} must be an object`);continue;}
        if(!territories.has(pyramid.territory_id))fail(`${label}.territory_id must reference a registered territory`);
        if(!sources.has(pyramid.source_id))fail(`${label}.source_id must reference a registered source`);
        if(!string(pyramid.period))fail(`${label}.period is required`);
        const key=JSON.stringify([pyramid.territory_id,pyramid.period]);if(pyramidKeys.has(key))fail(`${label} duplicates territory and period ${key}`);pyramidKeys.add(key);
        if(!Array.isArray(pyramid.bands)||pyramid.bands.length<3)fail(`${label}.bands needs at least three age groups`);
        else {
          const ages=new Set();
          for(const [bandIndex,band] of pyramid.bands.entries()){
            const bandLabel=`${label}.bands[${bandIndex}]`;
            if(!band||typeof band!=='object'||Array.isArray(band)){fail(`${bandLabel} must be an object`);continue;}
            if(!string(band.age))fail(`${bandLabel}.age is required`);else if(ages.has(band.age))fail(`${bandLabel}.age is duplicated`);else ages.add(band.age);
            for(const sex of ['male','female'])if(typeof band[sex]!=='number'||!Number.isFinite(band[sex])||band[sex]<0)fail(`${bandLabel}.${sex} must be a non-negative finite number`);
          }
        }
      }
    }

...
const DOCUMENT_TEMPLATE_STATUSES=['verified_prescribed_index','verified_requirements_based_outline'];
    ...
    if(!object(planning))fail('planning must be an object when supplied');
      ...
      if(own(planning,'document_template')) {
        ...
        if(!object(template))fail('planning.document_template must be an object');
          ...
          if(!DOCUMENT_TEMPLATE_STATUSES.includes(template.status))fail('planning.document_template.status must identify a verified prescribed index or a verified requirements-based outline');
          if(!checkedDate(template.verified_at))fail('planning.document_template.verified_at must be an ISO date or timestamp');
          ...
          if(!Array.isArray(template.sections)||!template.sections.length)fail('planning.document_template.sections needs at least one verified section');
            ...
            template.sections.forEach((section,index)=>{
              ...
              evidence(section.legal_basis,`${sectionPath}.legal_basis`);
      ...
      if(own(planning,'source_groups')) {
        if(!Array.isArray(planning.source_groups))fail('planning.source_groups must be an array');
          ...
          for(const id of template?.source_ids||[]){if(!lawIds.has(id))fail(`planning.document_template source ${id} must be classified in planning.source_groups law`);rawEvidence(id,'planning.document_template');}
          for(const [index,section] of (template?.sections||[]).entries())if(section?.legal_basis?.source_id&&!lawIds.has(section.legal_basis.source_id))fail(`planning.document_template.sections[${index}].legal_basis must use a law-group source`);

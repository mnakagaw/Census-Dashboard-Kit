...
const OUTPUTS=['docx','markdown','html','evidence_csv','documents_csv'];
const DOCUMENT_TEMPLATE_STATUSES=['verified_prescribed_index','verified_requirements_based_outline','provisional_evidence_outline'];
const SOURCE_GROUPS=['law','census','international'];
...
export function validatePlanning(data) {
  ...
  const territories=registry(data.territories),sources=registry(data.sources),indicators=registry(data.indicators);
  ...
  const evidence=(value,path)=>{
    ...
    if(own(value,'modality')&&!['mandatory','permissive','mixed','descriptive'].includes(value.modality))fail(`${path}.modality must be mandatory, permissive, mixed or descriptive`);
    if(value.modality==='mixed'&&!text(value.modality_note))fail(`${path}.modality_note must distinguish mandatory and discretionary elements when modality is mixed`);
  };
  ...
  if(own(data,'planning')) {
    ...
    if(!object(planning))fail('planning must be an object when supplied');
      ...
      if(own(planning,'document_template')) {
        const template=planning.document_template;
        if(!object(template))fail('planning.document_template must be an object');
        else {
          for(const key of ['title','authority','document_type','rationale'])required(template,key,'planning.document_template');
          if(!DOCUMENT_TEMPLATE_STATUSES.includes(template.status))fail('planning.document_template.status must identify a verified prescribed index, a verified requirements-based outline or a provisional evidence outline');
          const provisional=template.status==='provisional_evidence_outline';
          if(provisional){
            if(!checkedDate(template.reviewed_at))fail('planning.document_template.reviewed_at must be an ISO date or timestamp for a provisional evidence outline');
            required(template,'research_gap','planning.document_template');
          } else if(!checkedDate(template.verified_at))fail('planning.document_template.verified_at must be an ISO date or timestamp');
          for(const id of strings(template.source_ids,'planning.document_template.source_ids',{nonempty:true}))if(!sources.has(id))fail(`planning.document_template references unknown source ${id}`);
          if(!Array.isArray(template.sections)||!template.sections.length)fail('planning.document_template.sections needs at least one verified section');
          else {
            const seen=new Set();
            template.sections.forEach((section,index)=>{
              const sectionPath=`planning.document_template.sections[${index}]`;
              if(!object(section)){fail(`${sectionPath} must be an object`);return;}
              for(const key of ['id','number','title','guidance'])required(section,key,sectionPath);
              if(seen.has(section.id))fail(`${sectionPath}.id is duplicated`);seen.add(section.id);
              if(typeof section.required!=='boolean')fail(`${sectionPath}.required must be boolean`);
              const basis=provisional?section.evidence_basis:section.legal_basis;
              evidence(basis,`${sectionPath}.${provisional?'evidence_basis':'legal_basis'}`);
              if(section.required===true&&basis?.modality==='permissive')fail(`${sectionPath}.required cannot be true when its legal basis is permissive`);
              if(own(section,'indicator_ids'))for(const id of strings(section.indicator_ids,`${sectionPath}.indicator_ids`))if(!indicators.has(id))fail(`${sectionPath}.indicator_ids references unknown indicator ${id}`);
            });
          }
        }
      }
      if(own(planning,'source_groups')) {
        if(!Array.isArray(planning.source_groups))fail('planning.source_groups must be an array');
        else {
          const seen=new Set(),membership=new Map();
          planning.source_groups.forEach((group,index)=>{
            const groupPath=`planning.source_groups[${index}]`;
            if(!object(group)){fail(`${groupPath} must be an object`);return;}
            if(!SOURCE_GROUPS.includes(group.id))fail(`${groupPath}.id must be law, census or international`);
            if(seen.has(group.id))fail(`${groupPath}.id is duplicated`);seen.add(group.id);
            required(group,'label',groupPath);required(group,'note',groupPath);
            for(const id of strings(group.source_ids,`${groupPath}.source_ids`,{nonempty:true})){
              if(!sources.has(id))fail(`${groupPath} references unknown source ${id}`);
              if(membership.has(id))fail(`${groupPath} reuses source ${id} from ${membership.get(id)}; law, census and international roles need distinct source records`);
              else membership.set(id,group.id);
            }
          });
          const roleGroups=new Map(planning.source_groups.filter(group=>object(group)&&SOURCE_GROUPS.includes(group.id)).map(group=>[group.id,new Set(Array.isArray(group.source_ids)?group.source_ids:[])]));
          const lawIds=roleGroups.get('law')||new Set(),template=planning.document_template;
          for(const id of template?.source_ids||[]){if(!lawIds.has(id))fail(`planning.document_template source ${id} must be classified in planning.source_groups law`);if(template?.status!=='provisional_evidence_outline')rawEvidence(id,'planning.document_template');}
          const provisional=template?.status==='provisional_evidence_outline';
          for(const [index,section] of (template?.sections||[]).entries()){
            const basis=provisional?section?.evidence_basis:section?.legal_basis;
            if(basis?.source_id&&!lawIds.has(basis.source_id))fail(`planning.document_template.sections[${index}].${provisional?'evidence_basis':'legal_basis'} must use a law-group source`);
          }
          const seenArtifacts=new Map();
          for(const group of planning.source_groups){for(const id of group?.source_ids||[]){const source=sources.get(id);if(!source)continue;for(const [kind,value] of [['URL',source.url],['raw_path',source.raw_path],['SHA-256',source.sha256]]){if(!text(value))continue;const key=`${kind}:${String(value).trim().toLowerCase()}`,previous=seenArtifacts.get(key);if(previous&&previous.group!==group.id)fail(`planning source groups ${previous.group} and ${group.id} reuse the same ${kind} for ${previous.id} and ${id}`);else seenArtifacts.set(key,{group:group.id,id});}}}
        }
      }

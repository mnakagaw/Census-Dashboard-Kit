...
目的は、内部作業場所へ現地の地方統計・地域対応・計画資料を追加し、次の更新でも再現できるようにすること。World Bank全国値とgeoBoundaries参照境界は出発点であり、地方資料の調査を代替しない。
...
探索区分、行政所属、統計区分、計画主体を分ける。世界・大陸・広域の分類や国別の基礎自治体・細分停止点は、名称やADM番号だけで推測せず台帳と出典を持つ。全国を選択したときも内部の構成地域を比較できるか確認する。

各指標に対して、構成地域の全体台帳と比較粒度、同じ統計年・定義・母集団・単位・境界版の観測を用意する。欠測・秘匿・比較不可・地図未取得の地域を全件表から落とさず、地図と表で集合を一致させる。率の平均で親の値を作らず、親の公式値と検証した集計値を区別する。国勢調査の街区・集落等は必要な自治体値の根拠として調べても、通常UIの細分は基礎自治体で止める。
...
5. 別datasetの国版へ接続する場合だけ`country_sites`を設定し、`territory_id`、`country_id`、URL、移動先の`target_territory_id`と明示的な`indicator_map`を照合する。国際概念と国内統計は同名でも自動対応させず、世界へのデータ自動統合を前提にしない。

国際sourceを使う場合も、その参照図形を公式の国内統計境界と同一視しない。各資料の境界版、図形の性質、比較条件を示し、国内公式コードとの未照合を記録する。

下部比較の注目IDを上部の分析対象へ上書きしないこと、診断Markdown／HTML／全件CSVの地域・指標別最新版・出典年・内部全行・意味が対応すること、印刷が表の全行を保持することまで検証する。CSVは指標ごとの最新版と年の数値台帳、Markdownは編集可能な根拠表、HTMLは各指標の最新版による内部地図・全件表を含む。過去観測はデータ台帳に保持するが、国別3ページに時系列UIを置かない。既存の計画Markdown／HTML／根拠CSVの設定と独立して扱う。

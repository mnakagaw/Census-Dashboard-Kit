...
下部比較の注目IDを上部の分析対象へ上書きしないこと、診断Markdown／HTML／全件CSVの地域・全体値・内部全行・意味が対応すること、印刷が表の全行を保持することまで検証する。CSVは選択年の数値台帳、Markdownは編集可能な根拠表、HTMLは取得済み推移図と内部地図・全件表を含む。既存の計画Markdown／HTML／根拠CSVの設定と独立して扱う。

  ...
  "analysis": {
    ...
    "aggregation": {
      ...
      "rules": [{
        ...
        "label": "Dashboard calculated population",
...
- `geographic_level:national` 出典は国の値だけ。既存単一国のrootには `source.country_id` 未記載を許容する。国の全国値を地区へ割り当てることは許さない。
...
合成検証は `tests/analysis-fixture.mjs` と `tests/analysis.test.mjs`。複数階層、別branch、親欠測、0、同名市、30自治体を含む。

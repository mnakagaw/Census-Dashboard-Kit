...
既存の詳細所在調査がある国では、その国の国勢調査・計画制度・境界の候補を先に示す。検証済みレシピがある28件では、一般検索より先に、過去に成功した公式入口、API／Power BI／Redatam／archive等の取得方法、意味・地理上の注意を表示する。それ以外も全142件について、国別統計機関、UNSD国勢調査日程、FAOLEXの国別法令目録、JICA・外務省・UNDPの国別入口、geoBoundaries・UN SALB・HDX、WDI・MICS・DHS・WorldPop・GHSL・UNDP HDRの国別または共通入口を先に示す。いずれの場合も、リンクの発見や過去の取得成功は、現在の案件での取得・本文確認・地理照合・採用ではない。リンクの有効性、新版、法令改正、機関改編、データ内容、行政粒度、利用条件を案件時点で再確認する。

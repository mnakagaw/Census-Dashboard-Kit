...
Kit version 1.9.0 keeps `dataset.schema_version = "0.2"` for compatibility. Optional `analysis` and `planning` objects extend the schema without making old country datasets invalid. Version 1.1.0 added a one-shot delivery gate; version 1.2.0 added a verified country planning outline, DOCX output and classified source links. Version 1.3.0 added full acquired-table inventory, six-theme evidence coverage, selectable-level geography checks and language review. Version 1.4.0 makes country pages show the latest confirmed value and source year for each indicator, removes historical-period controls from those pages, and keeps thematic comparisons on one automatically selected common year. Version 1.5.0 added guided hierarchy selection, independent selected/parent/country map extents with zoom controls, and figure-rich DOCX diagnostic narratives. Version 1.5.1 keeps thematic geography stable when an indicator has no lower-area observations, permits map zoom-out below 100%, and guards national map rendering from missing-parent errors. Version 1.6.0 adds the source-backed 142-country/territory JICA priority registry and the distinct Dominican Republic, Uganda, Laos and Bangladesh reference cases to every country source preflight without promoting priority or example status to acquired evidence. Version 1.7.0 adds 28 audited Americas source recipes, keeps them at zero current-project acquisitions, and applies the stricter observed-depth and independent-audit rules learned from those builds. Version 1.8.0 adds a source-backed 250-country/area world identity and discovery registry (248 UN M49 rows plus explicit Taiwan and Kosovo operational supplements), multilingual country-name resolution, developed-country examples for Spain and Finland, and explicit register-based, devolved-law and structural-exception branches without storing statistical observations. Version 1.8.1 separates the latest completed UNSD census listing from the latest completed listing that has an UNSD link, and fixes current UNSD label matching for `St.`, Sint Maarten and numeric footnotes. Version 1.9.0 adds the validated AreaData-to-Kit source-feedback loop, deterministic merge registry and automatic country-preflight reuse without carrying AreaData adoption status into a new Kit project.
...
| `analysis.aggregation` | Only `exact_then_complete_cover` rules for declared indicators may calculate parents. Additive values use `sum`; rates use `weighted_mean` only with a declared source weight indicator for every non-overlapping member. The output retains components, weights, periods, missing IDs, and provenance. |
...
Thematic level choices follow the acquired territory hierarchy, including structurally valid levels with zero observations for the selected indicator. The thematic page keeps the **comparison area** separate from the **level shown inside that area** and provides controls for both. When an ADM1 or equivalent parent enters the thematic page, the nearest acquired lower level is selected while the parent remains the comparison scope. The map, table, median and coverage contain only that parent's members, retain the parent outline, and do not expand to the country when the indicator or level has no values. An all-missing level reports zero observed members and keeps every member unranked; it never silently switches to a level with data. Focusing a child in the map or ranking does not replace the comparison scope. A parent's direct missing value and an available child-area comparison are separate facts and must be labelled separately. A child comparison map never substitutes for the parent's value. Values calculated by the dashboard rather than published in the cited source are labelled **Dashboard calculated** on screen, CSV and Word.
...
The adopted Word output uses the title **Territorial Development Planning Diagnostic** for non-Spanish-speaking countries and **Diagnóstico Territorial para la Planificación del Desarrollo** for Spanish-speaking countries. For a non-Spanish-speaking country, its filename, title, headings, fixed prose and displayed indicator labels are English; DDPT Spanish and bilingual dataset labels do not leak into the document. Proper geographic names retain their source spelling. The output includes at least the Territorial Diagnostic, a location map, a population pyramid when age-sex data are available, available same-level comparison figures, latest acquired theme values with source years, the law/guidance-aligned index, and separate law, census/national-statistics and international-source lists. Narrative groups local gaps rather than repeating one empty sentence per indicator. It distinguishes selected-area observations, same-parent peer comparisons and broader-area context; broader values never populate the selected-area value. Detailed indicator tables are retained once in an annex. It is a diagnostic input and does not call itself an official plan or government form.
...
Visual completion also requires browser inspection at desktop and narrow widths. Within each control group, reserve a consistent label area and align the top and height of selects and inputs even when one label wraps. Put hierarchy search and its support action on a separate full-width row when placing them beside the hierarchy selectors would create gaps or uneven baselines. At 390px and the required narrow widths, stack these controls in reading order and verify their left and right edges, rather than relying only on a horizontal-overflow assertion.

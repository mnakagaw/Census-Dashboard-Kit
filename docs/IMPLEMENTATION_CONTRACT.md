...
Thematic level choices contain only levels with observed values for the selected indicator. The thematic page keeps the **comparison area** separate from the **level shown inside that area** and provides controls for both. When an ADM1 or equivalent parent enters the thematic page and the indicator has observations at a lower level, the nearest observed lower level is selected while the parent remains the comparison scope. The map, table, median and coverage contain only that parent's members, retain the parent outline, and do not expand to the country when the level changes. Focusing a child in the map or ranking does not replace the comparison scope. A parent's direct missing value and an available child-area comparison are separate facts and must be labelled separately. A child comparison map never substitutes for the parent's value. Values calculated by the dashboard rather than published in the cited source are labelled **Dashboard calculated** on screen, CSV and Word.
